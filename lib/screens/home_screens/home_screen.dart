...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return Directionality(
      ...
      child: Consumer(builder: (_, ref, __) {
        ...
        final bestSellingProducts =
            ref.watch(getProductOfSingleCategory(bestSellingId));
        final trendingProducts =
            ref.watch(getProductOfSingleCategory(trendingProductsId));
        final recommendedProducts =
            ref.watch(getProductOfSingleCategory(recommendedProductId));
        final specialOffers =
            ref.watch(getProductOfSingleCategory(specialOffersID));
                                ...
                                specialOffers.when(data: (snapShot) {
                                  return HorizontalList(
                                    ...
                                    itemBuilder:
                                      ...
                                      final productVariation = ref.watch(
                                          getSingleProductVariation(
                                              snapShot[index].id!.toInt()));

                                      return productVariation.when(
                                          data: (snapData) {
                                        ...
                                        return Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              MyGoogleText(
                                                text:
                                                    'USE CODE: ${snapShot[i].code.toString()}',
                                                fontSize: 16,
                                                fontColor: Colors.white,
                                                fontWeight: FontWeight.normal,
                                              ),
  ...
  Widget _buildCategoryList(List<CategoryModel> categories) {
    ...
    for (var element in categories) {
      if (element.parent != 0) {
        finalList.length < 8
            ? {finalList.add(element), allSubCategoryList.add(element)}
  ...
  Widget _buildCategoryList2(List<CategoryModel> categories) {
    ...
    for (var element in categories) {
      if (element.parent == 0) {
        finalList.length < 4 ? finalList.add(element) : null;
      }
    }
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
              image: DecorationImage(
                  fit: BoxFit.cover,
                  image: NetworkImage(finalList[0].image?.src ?? ''))),
          child: Text(
            finalList[0].name ?? '',
          ),
        ).onTap(() {
          SingleCategoryScreen(
            categoryId: finalList[0].id!.toInt(),
            categoryName: finalList[0].name.toString(),
            categoryList: finalList,
            categoryModel: finalList[0],
          ).launch(context);
        }),
        Container(
          height: 192,
          ...
          decoration: BoxDecoration(
              image: DecorationImage(
                  fit: BoxFit.cover,
                  image: NetworkImage(finalList[1].image?.src ?? ''))),
          child: Text(finalList[1].name ?? ''),
        ).onTap(() {
          SingleCategoryScreen(
            categoryId: finalList[1].id!.toInt(),
            categoryName: finalList[1].name.toString(),
            categoryList: finalList,
            categoryModel: finalList[1],
          ).launch(context);
        }),
        Container(
          height: 137,
          ...
          decoration: BoxDecoration(
              image: DecorationImage(
                  fit: BoxFit.cover,
                  image: NetworkImage(finalList[2].image?.src ?? ''))),
          child: Text(finalList[2].name ?? ''),
        ).onTap(() {
          SingleCategoryScreen(
            categoryId: finalList[2].id!.toInt(),
            categoryName: finalList[2].name.toString(),
            categoryList: finalList,
            categoryModel: finalList[2],
          ).launch(context);
        }),
        Container(
          height: 82,
          ...
          decoration: BoxDecoration(
              image: DecorationImage(
                  fit: BoxFit.cover,
                  image: NetworkImage(finalList[3].image?.src ?? ''))),
          child: Text(finalList[3].name ?? ''),
        ).onTap(() {
          SingleCategoryScreen(
            categoryId: finalList[3].id!.toInt(),
            categoryName: finalList[3].name.toString(),
            categoryList: finalList,
            categoryModel: finalList[3],
          ).launch(context);
        }),
      ],
    );
